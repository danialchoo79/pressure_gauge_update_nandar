...
def calibrate_gauge(filepath, output_path, line_key, pump_key):
    ...
    # image = cv2.imread('C:/nandar/analog-guage-reader/images/gauge-11.jpg')
    image = cv2.imread(filepath)
    # img_cw_90 = cv2.rotate(image, cv2.ROTATE_90_CLOCKWISE)
    # img = np.resize(image,(271, 262))
    ...
    # circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, 20, np.array([]), 100, 50, int(height*0.25), int(height*0.46))
    # circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1.5, 5, np.array([]), 100, 50, int(height*0.25), int(height*0.46))
...
def get_current_value(
    ...
    final_indx = line_length_lst.index(max(line_length_lst))
    final_line_list.append(line_pos_lst[final_indx])
